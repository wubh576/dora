...
屏幕顶部中央常驻一个原生 AppKit 顶部控制条，不占用 macOS 菜单栏图标位，也不创建 Dock 图标。紧凑态固定显示左侧 `Dora` 和右侧活跃 session 总数：有 waiting 时数字为红色，只有 running 时为蓝色，无活跃 session 时为灰色；展开后再显示 waiting/running 的准确拆分。控制条在普通屏约为 280 pt 宽，刘海屏则读取系统顶部左右辅助区域，为摄像头保留真实宽度并在两侧各留一个等宽内容区，因此系统截图看不到的物理遮挡也不会压住文字。高度跟随当前主屏实际菜单栏上下边界，菜单栏自动隐藏时依次使用屏幕安全区与系统菜单栏厚度。鼠标进入和离开由 `NSTrackingArea` 与 local/global `NSEvent` mouse monitor 事件驱动，panel frame 变化完成后再单次复查；鼠标静止时没有固定频率的位置轮询。停留约 100–150 ms 后展开今日、近 7 日、近 30 日、全部 token、Codex 5 小时/7 日配额和 waiting/running 会话；右上角使用 SF Symbols 提供刷新、仪表盘、设置和退出四个图标按钮，底栏左侧显示 waiting/running 计数，右侧显示快照或临时操作状态。近 7 日与近 30 日都按 macOS 本地日历日计算，分别包含今天和此前 6 日或 29 日，并截止当前时刻。新 attention request 会播放一次 `Glass`、自动展开约 6 秒并高亮对应会话；点击可精确定位的会话后，面板会在跳转返回前保持展开，成功后立即收起，失败时继续展开并在底部显示真实原因，不会把请求标记为已处理。

刘海屏和普通屏都让控制条顶边精确贴住主屏幕顶边，只保留下方两个圆角。展开后只要鼠标仍在整个 panel 内就保持展开；离开整个区域约 450 ms 后才收起，延迟到期还会用当前屏幕坐标复查鼠标是否仍在面板内。按钮交互覆盖 AppKit 完整的鼠标 tracking loop，不能依赖可能被控件消费的独立 mouse-up 事件；因此点击刷新后可留在面板内查看进度和结果，但刷新不会阻止鼠标离开后的正常收起。刷新期间图标显示原生进度并禁止重复点击，底栏状态继续经历“正在刷新数据…”、刷新结果和约 10 秒后恢复快照状态的既有链路。成功打开仪表盘后则立即收起。设置图标打开一个可复用的普通 macOS 窗口，目前只显示“设置项将在后续加入”，没有真实设置项或持久化。会话较多时只滚动中间列表，顶部和底栏保持可见。退出仍走同一个 Go 优雅关闭路径，关闭 HTTP 服务并释放端口。
...
Codex App 的工具授权可以在灵动岛中点击“处理授权”，查看本次工具的完整 JSON 参数及工作目录，再选择“本次允许”“本次拒绝”或“转到原应用”。只处理仍有同步 Hook 连接的请求；同一会话的多个请求需要逐条选择，不提供永久允许或权限规则修改。普通问题、CLI 和 WorkBuddy 继续在原应用中处理。

审批最多等待两分钟；Dora 不运行、界面停止更新、请求取消或超时后不作决定，交回 Codex 原有权限流程，不会自动允许。决定发送后也不代表工具已执行成功，运行状态仍由 Codex 后续事件更新。新增功能会调整 `PermissionRequest` 超时并注册 `Interrupt`，因此升级后需在 Codex `/hooks` 中重新检查并信任这两项配置。审批详情仅在内存和受 control token/Origin 保护的本地接口中短暂存在，不进入 SQLite、日志或普通 runtime API。
...
Hook helper 只向固定的 `127.0.0.1:8080` 发送有界 JSON，不跟随重定向。普通事件发送脱敏字段；仅 App 审批在等待期间通过独立受保护接口发送工具参数。Dora 不持久化完整 prompt、回复、完整命令、工具参数、环境变量、transcript 路径或完整 cwd；Codex App 输入会先去掉 `## My request for Codex:` 之前的 IDE、文件和浏览器上下文，再对真正的用户请求保存去控制字符、压缩空白且最多 160 个 Unicode 字符的一行摘要，用于活跃会话展示，并在 root `Stop`、新 `SessionStart` 或 `SessionEnd` 时清除。活跃任务标题按 external session ID 从 Codex App 的 `state_5.sqlite` 只读获取，并缓存到 Dora SQLite 的临时 runtime 记录；读取不到时使用已缓存标题，再回退到 cwd basename。SQLite 的临时 runtime 记录还包含 external session ID、cwd basename、模型、App/CLI surface、受支持终端的精确 TTY 和等待状态；运行态 API 不返回这些原始定位值，只返回任务标题、`jumpable` 与 `jumpReason`。`SessionEnd` 或跳转确认目标已消失时会连同缓存标题移除 runtime session；Dora 重启时把上次遗留的 running 恢复为 idle，保留尚未解决的 waiting，但不会对历史请求重复发声。

Codex subagent 不会在 Dora 中形成独立 session。标准 `PermissionRequest` 和 `PostToolUse` 可能只能确认父 session，不能确认具体 child；这类请求显示为“Codex 等待授权”，按每个请求分别提醒，父标题、prompt、模型、surface、TTY、cwd 和跳转目标保持不变。Dora 为授权与工具完成事件生成不可逆 `tool_input_key`：Bash 只使用真正决定执行身份的 `command`，不会让授权专用的顶层 `description` 阻断关联；MCP 和其他工具仍使用完整规范化输入，避免忽略真实业务参数。完整输入指纹继续用于既有事件去重兼容，普通事件不携带原始工具输入；App 交互审批的详情仅经过独立受保护接口。精确键均不可用时，只在同 parent/scope/turn/tool/kind 候选唯一时回落解除，多个候选会全部保留。事件真实携带 `agent_id` 时才生成 child scope 并显示“Subagent 等待授权”；`SubagentStop` 也只清理相同非空 scope。点击会话行仍回到父 Codex 会话；存在存活的 App 审批连接时，可通过“处理授权”单独作出本次决定。Codex App 固定 `# Overview` Ambient Suggestions 后台 prompt 仍会被排除。上述规则只影响实时会话与提醒，后台模型实际消耗仍计入 token 和费用。

PermissionRequest 不存在假想的即时 resolved 回调。Codex CLI `0.146.0` 实测 root Allow 后最早在 `PostToolUse` 解除；按 Esc 取消后没有即时完成事件，最早在下一次 root `UserPromptSubmit` 解除。root `Stop` 或 `SessionEnd` 会清理该父 session 的 waiting；child 完成只清理自己的 waiting。进程异常退出时，瞬时 running 不跨重启延续；未解决的 waiting 继续展示，超过 7 天没有任何 Hook 活动后由最终 stale reconciliation 清理，不会用几秒钟的盲目 timeout 提前解除真实等待。
