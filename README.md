...
屏幕顶部中央常驻一个原生 AppKit 顶部控制条，不占用 macOS 菜单栏图标位，也不创建 Dock 图标。紧凑态固定显示 `Dora` 和 waiting/running session 数，存在 waiting 时数字变红；高度跟随当前主屏实际菜单栏上下边界，菜单栏自动隐藏时依次使用屏幕安全区与系统菜单栏厚度。AppKit 主线程持续判断鼠标是否进入紧凑条，停留约 100–150 ms 后展开今日、7 日、全部 token、Codex 5 小时/7 日配额、waiting/running 会话、刷新、仪表盘和退出。新 attention request 会播放一次 `Glass`、自动展开约 6 秒并高亮对应会话；点击可精确定位的会话后，面板会在跳转返回前保持展开，成功后立即收起，失败时继续展开并在底部显示真实原因，不会把请求标记为已处理。

刘海屏和普通屏都让控制条顶边精确贴住主屏幕顶边，只保留下方两个圆角。展开后只要鼠标仍在整个 panel 内就保持展开；离开整个区域约 450 ms 后才收起，timer 到期还会用当前屏幕坐标复查鼠标是否仍在面板内。会话较多时只滚动中间列表，头部统计与底部操作保持可见。退出会关闭 HTTP 服务并释放端口。
