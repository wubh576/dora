...
刘海屏和普通屏都让控制条顶边精确贴住主屏幕顶边，只保留下方两个圆角。展开后只要鼠标仍在整个 panel 内就保持展开；离开整个区域约 450 ms 后才收起，延迟到期还会用当前屏幕坐标复查鼠标是否仍在面板内。点击刷新后保持展开以查看进度和结果，鼠标离开后再收起；成功打开仪表盘后则立即收起。会话较多时只滚动中间列表，头部统计与底部操作保持可见。退出会关闭 HTTP 服务并释放端口。
