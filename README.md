...
Codex subagent 不会在 Dora 中形成独立 session。它的 `PermissionRequest` 会作为“Subagent 等待授权”聚合到父 session，按每个请求分别提醒一次；父标题、prompt 和精确跳转定位保持不变，点击仍只回到父 Codex 会话，允许或拒绝继续由 Codex 原生界面处理。child 的 `PostToolUse`、`Stop` 或 `SubagentStop` 只解除同一 child 的请求，不影响其他 child 或 root request；其他 child 生命周期成功忽略，不参与父 runtime 状态迁移。child scope 和工具调用只保存不可逆内部 key，不会通过 Runtime API 暴露。Codex App 固定 `# Overview` Ambient Suggestions 后台 prompt 仍会被排除。上述规则只影响实时会话与提醒，后台模型实际消耗仍计入 token 和费用。
