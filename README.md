...
Hook helper 只向固定的 `127.0.0.1:8080` 发送有界、脱敏 JSON，不跟随重定向。Dora 不持久化或记录完整 prompt、回复、完整命令、工具参数、环境变量、transcript 路径或完整 cwd；当前被 `PermissionRequest` 阻塞的请求只在内存中保留工具名、可选描述和已经去凭证、压缩空白、截断为一行的操作摘要。Codex App 输入会先去掉 `## My request for Codex:` 之前的 IDE、文件和浏览器上下文，再对真正的用户请求保存去控制字符、压缩空白且最多 160 个 Unicode 字符的一行摘要，用于活跃会话展示，并在 `Stop`、新 `SessionStart` 或 `SessionEnd` 时清除。活跃任务标题按 external session ID 从 Codex App 的 `state_5.sqlite` 只读获取，并缓存到 Dora SQLite 的临时 runtime 记录；读取不到时使用已缓存标题，再回退到 cwd basename。SQLite 的临时 runtime 记录还包含 external session ID、cwd basename、模型、App/CLI surface、受支持终端的精确 TTY 和等待状态；运行态 API 不返回这些原始定位值，只返回任务标题、`jumpable` 与 `jumpReason`。`SessionEnd` 或跳转确认目标已消失时会连同缓存标题移除 runtime session；Dora 重启时把上次遗留的 running 恢复为 idle，保留尚未解决的 waiting，但不会对历史请求重复发声。运行列表只由用户主动提交的 root prompt 启动；Codex 0.146.0 标记了 `agent_id` 或 `agent_type` 的 subagent 事件会转换成最小结束事件，Codex App 固定 `# Overview` Ambient Suggestions 后台 prompt 也会被排除。该规则只影响实时会话与提醒，后台模型实际消耗仍计入 token 和费用。

实时 `PermissionRequest` 会在对应 waiting 行显示原生分段按钮：`本次允许` 只允许当前请求，箭头菜单提供 `拒绝` 和 `在 Codex 中处理`。前两项直接通过官方 Hook stdout 协议返回一次性决定，不切换前台，也不会创建 session 级或永久规则；交回处理和点击该行都会先让当前 Hook 以空 stdout 成功退出，再使用现有精确定位跳转到 Codex App、iTerm2 或 Terminal。相同命令、同一 session 的连续请求仍各有独立 interaction ID，处理一条不会唤醒其他 waiter；动作被接受后按钮立即消失，队列中的下一条随后出现。`request_user_input`、历史 waiting 和 running 行不会显示该按钮。

`PermissionRequest` handler 配置为 600 秒，Dora 内部最多等待约 570 秒；其他 Hook 仍使用 1～2 秒短超时。Dora 停止、重启、断开、超时或本地服务不可用时，helper 都以退出 0、空 stdout 交回 Codex 原生授权提示。实现只使用公开 Hook 协议，不使用 UI 自动化、Codex App Server 或私有接口。由于 timeout 是 Hook 定义的一部分，本次变更后需要重新在 Codex `/hooks` 检查并授权新的 hash；Dora 不会自动写入 trust hash。
