...
- 点击刷新后只要鼠标仍在控制条内就保持展开，图标替换为原生进度指示但按钮保持 enabled，底栏右侧继续直接展示刷新进度和结果。刷新中再次点击只保留按压反馈，原生 action 和 Controller 都会忽略该请求，不排队也不启动第二轮；刷新本身不是强制展开理由，鼠标离开整个区域后即按统一的 450 ms 延迟收起，不等待扫描或配额请求结束。
